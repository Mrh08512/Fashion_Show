...
@interface MJPhotoBrowser () <MJPhotoViewDelegate,UIActionSheetDelegate>
{
    ...
    UIImage *willSaveToPhotoImage;
    
    BOOL isDidShowActionSheet;
}
@end
...
- (id)init
{
    ...
    if (self)
    {
        ...
        isDidShowActionSheet = NO;
    }
    return self;
}


#pragma mark 用户注销登录的通知action
- (void)userLogoutRemovePhotBrowerNotificationAction:(NSNotification *)notifation
{
...
- (void)longTapPhotoFinish:(MJPhotoView *)photoView
{
    if (isDidShowActionSheet)
    {
        return;
    }
    
    isDidShowActionSheet = YES;
    
    if (photoView.photo.image)
    {
        willSaveToPhotoImage = photoView.photo.image;
        
        UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@"" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"保存到手机", nil];
        [actionSheet showInView:[UIApplication sharedApplication].keyWindow];
    }else
    {
        willSaveToPhotoImage = nil;
    }
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex == 0)
    {
        if (willSaveToPhotoImage)
        {
            UIImageWriteToSavedPhotosAlbum(willSaveToPhotoImage, self,
...
(NSError *)error contextInfo:(void *)contextInfo;
{
    ...
    if (!error)
        ShowAlertView(@"", @"保存成功", @"OK", nil);
    else
        ShowAlertView(@"", [NSString stringWithFormat:@"Error writing to photo album: %@",
                                                       [error localizedDescription]], @"OK", nil);
}
